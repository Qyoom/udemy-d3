...
/*
  ...
  which is why a nested element (text label) can be shifted in a negative direction (i.e.
  outside the boundary of the parent group element.)
  Note that each element is positioned in respect to its parent's origin, but along it's own
  x and y dimensions (important when rotation occurs). Correspondingly each element has its
  own origin pertanent to the position of any nested (immediate child) elements.
  >>>
  [A translation (or x,y coordinate position)] can be interpreted as shifting the origin 
